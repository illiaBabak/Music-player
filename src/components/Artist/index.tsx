...
export const Artist = ({ artist }: Props): JSX.Element => {
  ...
  return (
    <Card
      className={`artist p-2 m-2 ${isLightTheme ? "light" : "dark"} ${selectedChip === "All" ? "line" : ""}`}
    >
      {!!artist.images.length && (
        <Card.Img src={artist.images[0].url} className="artist-icon" />
      )}
      <span className="m-3 title">{artist.name}</span>
      {selectedChip !== "All" && (
        <Card.Body className="d-flex flex-column artist-body">
          {!!artist.genres.length && (
            <div className="fs-6">Genre: {artist.genres[0]}</div>
          )}
